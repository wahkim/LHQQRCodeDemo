...
/**
 扫码区域动画效果
 */
typedef NS_ENUM(NSInteger,LHQScanViewAnimationStyle)
{
    LHQScanViewAnimationStyle_LineMove,   //线条上下移动
    LHQScanViewAnimationStyle_NetGrid,//网格
    LHQScanViewAnimationStyle_LineStill,//线条停止在扫码区域中央
    LHQScanViewAnimationStyle_None    //无动画
...
/**
 扫码区域4个角位置类型
 */
typedef NS_ENUM(NSInteger, LHQScanViewPhotoframeAngleStyle)
{
    LHQScanViewPhotoframeAngleStyle_Inner,//内嵌，一般不显示矩形框情况下
    LHQScanViewPhotoframeAngleStyle_Outer,//外嵌,包围在矩形框的4个角
    LHQScanViewPhotoframeAngleStyle_On   //在矩形框的4个角上，覆盖
};

NS_ASSUME_NONNULL_BEGIN

@interface LHQScanViewStyle : NSObject
...
/**
 @brief  扫码区域的4个角类型
 */
@property (nonatomic, assign) LHQScanViewPhotoframeAngleStyle photoframeAngleStyle;
...
/**
 @brief  扫码动画效果:线条或网格
 */
@property (nonatomic, assign) LHQScanViewAnimationStyle anmiationStyle;
