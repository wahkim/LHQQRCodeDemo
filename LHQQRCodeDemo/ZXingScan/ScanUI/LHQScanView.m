...
@interface LHQScanView ()

//扫码区域
@property (nonatomic,assign)CGRect scanRetangleRect;

//线条扫码动画封装
@property (nonatomic, strong) LHQScanLineAnimation *scanLineAnimation;
//网格扫码动画封装
@property (nonatomic, strong) LHQScanNetAnimation *scanNetAnimation;
//线条在中间位置，不移动
...
- (void)startScanAnimation
{
    ...
    {
        case LHQScanViewAnimationStyle_LineMove:
        {
            //线动画
            if (!_scanLineAnimation)
                self.scanLineAnimation = [[LHQScanLineAnimation alloc]init];
            [_scanLineAnimation startAnimatingWithRect:_scanRetangleRect
                                                InView:self
                                                 Image:_viewStyle.animationImage];
        }
            break;
        case LHQScanViewAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation)
                self.scanNetAnimation = [[LHQScanNetAnimation alloc]init];
            [_scanNetAnimation startAnimatingWithRect:_scanRetangleRect
                                               InView:self
                                                Image:_viewStyle.animationImage];
        }
            break;
        case LHQScanViewAnimationStyle_LineStill:
        {
            if (!_scanLineStill) {
...
- (void)drawScanRect
{
  ...
  //画矩形框4格外围相框角
    ...
    {
        case LHQScanViewPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
            break;
        case LHQScanViewPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
            break;
        case LHQScanViewPhotoframeAngleStyle_Inner:
        {
...
+ (CGRect)getZXingScanRectWithPreView:(UIView*)view style:(LHQScanViewStyle*)style
{
    ...
    NSLog(@"扫描区域%@", NSStringFromCGRect(cropRect));
