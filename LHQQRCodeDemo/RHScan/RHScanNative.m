...
- (instancetype)initWithPreView:(UIView*)preView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect
                       delegate:(id<RHScanNativeDelegate>)delegate {
    if (self = [super init]) {
        [self initParaWithPreView:preView ObjectType:objType cropRect:cropRect delegate:delegate];
    }
    return self;
}

- (void)initParaWithPreView:(UIView*)videoPreView ObjectType:(NSArray*)objType cropRect:(CGRect)cropRect delegate:(id<RHScanNativeDelegate>)delegate
{
    ...
    [self.videoPreView addSubview:self.focusView];
...
//         }
        if ([weakSelf.delegate respondsToSelector:@selector(scanNative:result:)]) {
            [weakSelf.delegate scanNative:weakSelf result:weakSelf.arrayResult];
        }
...
- (void)focusAtPoint:(CGPoint)point {
    CGSize size = self.videoPreView.bounds.size;
...
/// 设置平滑对焦
- (void)setSmoothAutoFocus:(BOOL)isSmoothFocus {
    NSError *error;
    if ([self.device lockForConfiguration:&error]) {
        if ([_device isSmoothAutoFocusSupported]) {
            _device.smoothAutoFocusEnabled = isSmoothFocus;
        }
        [self.device unlockForConfiguration];
    }
...
- (void)changeFactor:(CGFloat)currentZoomFactor {
    if (currentZoomFactor < self.maxZoomFactor + 1 &&
        ...
        NSError *error = nil;
        if ([self.device lockForConfiguration:&error] ) {
            [self.device rampToVideoZoomFactor:currentZoomFactor withRate:5];
            [self.device unlockForConfiguration];
...
//        }
        if ([self.delegate respondsToSelector:@selector(scanNative:result:)]) {
            [self.delegate scanNative:self result:self.arrayResult];
        }
    }
}
